...
class DailyLedger(Base):
    ...
    product_id = Column(UUID(as_uuid=True), ForeignKey("playauto_platform.products.id"), nullable=False)
    ...
    # 날짜와 제품별로 유니크
    __table_args__ = (
        UniqueConstraint('ledger_date', 'product_id', name='uq_ledger_date_product'),
